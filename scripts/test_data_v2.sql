-- ============================================
-- 趣享社测试数据生成脚本 v2
-- 执行方式: mysql -u root -p123456 quxiangshe < test_data_v2.sql
-- ============================================
...
-- 更新笔记 stable_random
UPDATE note SET stable_random = id * 0.000001 + RAND() * 0.1 WHERE stable_random IS NULL OR stable_random = 0;
...
BEGIN
  ...
  WHILE i <= 1000 DO
    ...
    WHILE cnt > 0 DO
      ...
      END;
      INSERT IGNORE INTO follow (follower_id, following_id, created_at) VALUES (i, target, DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 30) DAY));
      SET cnt = cnt - 1;
    END WHILE;
    SET i = i + 1;
...
SELECT DISTINCT FLOOR(1 + RAND() * 50), FLOOR(302 + RAND() * 1000), DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 30) DAY)
FROM (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10) AS a,
     (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10) AS b,
     (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5) AS c
WHERE RAND() < 0.5;
...
SELECT FLOOR(302 + RAND() * 100), 1, FLOOR(2 + RAND() * 20), FLOOR(1 + RAND() * 50), '赞了你的笔记', 0, DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 7) DAY)
FROM (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5) AS a;

INSERT INTO notification (user_id, type, from_user_id, note_id, content, is_read, created_at)
SELECT FLOOR(302 + RAND() * 100), 2, FLOOR(2 + RAND() * 20), FLOOR(1 + RAND() * 50), '评论了你的笔记', 0, DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 7) DAY)
FROM (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5) AS a;

INSERT INTO notification (user_id, type, from_user_id, content, is_read, created_at)
SELECT FLOOR(2 + RAND() * 100), 3, FLOOR(302 + RAND() * 200), '关注了你', 0, DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 7) DAY)
FROM (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5) AS a;
